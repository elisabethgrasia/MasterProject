# -*- coding: utf-8 -*-
"""
Created on Mon May 15 22:21:33 2023
...
def conv_block(inputs, filters, kernel_size, dropout=0.0, num_conv_layers=1, pool_type='conv', pool_size=2, residual=False):
    x = inputs 
    for i in range(num_conv_layers):
        
        if str(pool_type).lower().startswith('conv') and i == num_conv_layers-1:
            x = layers.Conv1D(filters, kernel_size, strides=pool_size, padding='same')(x)
        else:
