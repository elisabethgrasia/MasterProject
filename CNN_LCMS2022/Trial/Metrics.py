# -*- coding: utf-8 -*-
"""
Created on Sat May 13 10:13:40 2023
...
class CustomTruePositiveRate(tf.keras.metrics.Metric):
    def __init__(self, threshold, **kwargs):
        ...
        self.false_negatives = self.add_weight(name='tp', initializer='zeros')   # name tp???
...
class CustomFalsePositiveRate(tf.keras.metrics.Metric):
    ...
    def __init__(self, threshold, **kwargs):
        ...
        self.true_negatives = self.add_weight(name='tp', initalizer='zeros')
...
class CustomMREArea(tf.keras.metrics.Mean):
    ...
    def update_state(self, y_true, y_pred, sample_weight=None):
        ...
        mask = tf.math.logical_and(tf.math.equal(y_true_prob, 1.), tf.math.greater(y_pred_prob, 0.5))
        
        y_true = tf.boolean_mask(y_true, mask)
        y_pred = tf.boolean_mask(y_pred, mask)
        
        error = tf.math.divide_no_nan(tf.math.abs(y_true - y_pred), tf.math.abs(y_true))
