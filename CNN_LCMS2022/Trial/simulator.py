# -*- coding: utf-8 -*-
"""
Created on Mon May 15 23:02:01 2023
...
def asymmetrical_gaussian_peak(x, amplitude, loc, s0, s1, s2=0.0, eps=1e-7):
    # asymmetrical_gaussian_peak(..., s1=0) is the same as gaussian_peak(...)
    # add additional asymmetric by adding to the denominator inside np.exp:
...
class Simulator:
    ...
    def __init__(
        ...
        self.asymmetric_range = asymmetric_range
    ...
    def _get_random_asymmetries(self, locs, n_peaks):
        asymmetry_loc = np.random.uniform(*self.asymmetric_range)
    ...
    def _generate_example(self, random_seed):
        ...
        ## b. Remove collisions:
            ...
            # Otherwise, we will train the model to predict one peak
            # (for the given heavily-overlapping peak) even though there are two peaks
        if self.remove_collision_fn is not None:
            locs, scales, s1s, s2s, ampls = self.remove_collision_fn(
                locs, scales, s1s, s2s, ampls)
        
        ## c. Generate (noise-free) chromatogram from peaks (and obtain areas)
        ...
        # chromatogram = apply_baseline_drift(chromatogram, self.resolution, self.baseline_drift_magnitude)
