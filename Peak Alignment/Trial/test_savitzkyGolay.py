# -*- coding: utf-8 -*-
"""
Created on Thu Apr  6 14:52:19 2023
...
def alt_sg_coeffs(window_length, polyorder, pos):
    ...
    It uses numpy.polyfit and numpy.polyval. The results should be equivalent
    to those of savgol_coeffs(), but this implementation is slower.
...
def test_sg_coeffs_exact():
    ...
    # SG filter, so the filtered value should equal to the input data
    ...
    # Check the same input, but use deriv=2. dy2 is the exact result.
    d2y = 3.0 * x
...
def test_sg_coeffs_large():
    ...
    assert_array_almost_equal(coeffs1, -coeffs2[::-1])
...
def test_sg_filter_interp_edges():
    # Another test with low degree polynomial data, for which we can
    # easily give the exact results. In this test, we use mode='interp',
    # so savgol_filter should match the exact solution for the entire
    # datset, including the edges.
...
def test_sg_filter_interp_edges_3d():
    # Test mode='interp' with a 3D array
    t = np.linspace(-5, 5, 21)
    delta = t[1] - t[0]
    x1 = np.array([t, -t])
