...
from partitionSignal import partitionSignal
...
def GCPeakDetection(gcData, timesteps, splitSignalSize,
    ...
    if splitSignalSize == 0:
        ...
        partStIndex, partEndIndex = partitionSignal(smdInitial, thrSig, N, splitSignalSize)
    ...
    NPeaksTotal = 0
    for partNumb in range(NPartition):
        
        # get the data associated with this section
        dataPartOrig = gcData[partStIndex[partNumb-1] : partEndIndex[partNumb]]
        smdInitPart = smdInitial[partStIndex[partNumb-1]:partEndIndex[partNumb]]
        NDataPart = len(dataPartOrig)
        timePart = timesteps[partStIndex[partNumb-1]: partEndIndex[partNumb]]
        ...
        if any(smdInitPart - thrSig > 0):
            ...
            for wsi in range(len(windowSize)):
                ...
                valDW[wsi] = np.divide(np.sum((diffSmdOrig[1:] - diffSmdOrig[:-1]) ** 2), np.sum([(x ** 2) for x in diffSmdOrig]) * (NDataPart / (NDataPart - 1)))
            ...
            if showPlot:
                ...
                hFigPart.clf()
                plt.title(f"Partition Number {partNumb}", fontsize=14)
        ...
        # if any(smdZD - thrSig > 0)
            ...
            if showPlot:
                ...
                plt.plot([timePart[0], timePart[-1]], [thrSig, thrSig], '--', linewidth=1, color=[0.33,0, 0])
                plt.show()
            ...
            if indSD:
                ...
                # continues indices
                indIndSD_NegZoneEnd = np.concatenate([np.where(np.diff(indSD) > 1)[0], [indSD - 1]])
        ...
        # need a threshold for the first derivative for start and end of peak
            ...
            if showPlot:
                plt.figure(hFigPart)
                plt.plot([timePart[0], timePart[-1]], [thrFD, thrFD], '--', linewidth=1, color=[0, 0.33, 0])
                plt.plot([timePart[0], timePart[-1]], [-thrFD, -thrFD], '--', linewidth=1, color=[0, 0.33, 0])
                plt.show()
            ...
            for nZone in range(nSDNegZones):
                ...
                indMaxZD = np.argmax(smdZDinNegZone)
                ...
                elif timePart[indPeakMax] > resPeaksPart[numbPeaksPart, colPeakEnd]:
                    prevPeakEndB4CurrPeak = True
                ...
                if (all(smdZDinNegZone > thrSig)) and (all(abs(smdFDinNegZone) > thrFD)) and (len(smdZDinNegZone) > 1):
                    ...
                    if not prevPeakEndB4CurrPeak:
                        ...
                        while True:
                            if indTemp > indSD_NegZoneSta[nZone]:
                                ...
                                break
                            elif np.diff(smdTD[indTemp:indTemp+1] > 0).item() == 0:
                                indTemp += 1
                            else:
                                indEndPrevSD = indTemp+1
                                break
                            
                        resPeaksPart[numbPeaksPart-1, colPeakEnd] = timePart[indEndPrevSD]
                    ...
                    resPeaksPart[numbPeaksPart, colPeakStart] = timePart[indFD_startOfPeak]
                    ...
                    resPeaksPart[numbPeaksPart, colPeakEnd] = timePart[indFD_endOfPeak]
                    ...
                    if nDipsInSD > 1 and splitCoelution:
                        ...
                        mpi1 = np.zeros(nDipsInSD, dtype=int)
                        mpi2 = np.zeros(nDipsInSD, dtype=int)
                    ...
                    indPeakMaxInMultiZone = indSD_NegZoneSta[nZone] + indMinsSD
                    ...
                    indPeakSplit = indSD_NegZoneSta[nZone] + indMaxsSD
                    ...
                    for nDip in range(nDipsInSD):
                        multiPeakZoneRes[nDip, colPeakArea] = calcAUC(smdZD[mpi1[nDip]:mpi2[nDip]],timePart[mpi1[nDip]:mpi2[nDip]])
                    ...
                    if (nDipsInSD == 1):
                     ...
                     ## ===== STEP 3f. b. calculate the area under the curve
                        ...
                        if len(smdZD[indFD_startOfPeak:indFD_endOfPeak]) > 1:
                            areaPeak = calcAUC(smdZD[indFD_startOfPeak:indFD_endOfPeak], timePart[indFD_startOfPeak:indFD_endOfPeak])
            ...
            if showPlot:
                plt.figure(hFigPart)
                plt.plot(resPeaksPart[:, colPeakStart], np.zeros(numbPeaksPart),'m^', markersize=12, markerfacecolor= 'm')
                plt.plot(resPeaksPart[:, colPeakEnd], np.zeros(numbPeaksPart), 'rv', markersize=12, markerfacecolor= 'r')
                plt.plot(resPeaksPart[:, colPeakMax], np.zeros(numbPeaksPart), 'go', markersize=12, markerfacecolor= 'g')
                plt.show()
